...
function render(data){
  ...
  shell.insertAdjacentHTML('beforeend', `<section class="schedule-card"><h2>Data Layer Ready</h2><p>${esc(data.summary || fallbackData.summary)}</p><p class="schedule-muted">v5.14.0 uses graceful fallback data so missing preview JSON no longer displays alarming fetch errors.</p></section>`);
}
